...
def move(exits, direction):
    ...
    False
    """
    room_choice = [exits][0][direction]
    new_choice = rooms[room_choice]
    return new_choice

def main():

    starting = rooms["Reception"]
    while True:
        display_room(starting)
        exits = starting["exits"]
        direction = menu(exits)
        starting = move(exits, direction)
